...
def caesar_cipher(color_sequence, step):
    ren_mass = list(map(chr, list(range(ord('a'), ord('g'))))) + list(map(chr, list(range(ord('0'), ord('9') + 1))))
    ren = ''.join(ren_mass)
    return color_sequence.translate(str.maketrans(ren[step:] + ren[:step], ren))
...
def decrypt(key, width, height, obj, step):
    md5_key = make_md5(key)
    reverse_key = list(md5_key)[::-1]
    #for s in range(len(reverse_key)):
    #    if s+1 < len(reverse_key):
    #        if reverse_key[s] == reverse_key[s+1]:
    #            del reverse_key[s]
    distortion = -1
    for s in reverse_key:
	    i = s.find('0')
	    if i > -1:
		    distortion += 1
    is_aply = 0
    for a in reverse_key:
            ...
            for i in range(height):
                for j in range(width):
                    if l == 0:
                        ...
                        b = obj[j, i][2]
                        if r - distortion < 0:
                            r = int(math.fabs(r-distortion))
                        else:
                            r = int(math.fabs(r+distortion))
                        if g - distortion < 0:
                            g = int(math.fabs(g-distortion))
                        else:
                            g = int(math.fabs(g+distortion))
                        if   b - distortion < 0:
                            b = int(math.fabs(b-distortion))
                        else:
                            b = int(math.fabs(b+distortion))
                        obj[j, i] = (r, g, b)
                        continue
